...
프로젝트가 깨끗해질 때까지 멈추지 않는 끈질긴 검증 루프를 실행합니다: **사용자 요청의 인자**

`$unity-workflow`가 검증기를 한 번 실행하는 것(내부적으로 최대 3회 반복)과 달리, ralph는 남은 문제가 0개가 되거나 안전 한도에 도달할 때까지 검증기를 반복해서 실행합니다.

## 설정

`사용자 요청의 인자`에서 옵션을 파싱합니다.
...
1. **`unity-verifier` 에이전트 호출** — collaboration.spawn_agent 도구를 통해 다음과 함께 호출합니다.
   - 초점 범위 (제공된 경우)
