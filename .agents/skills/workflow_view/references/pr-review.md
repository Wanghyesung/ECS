...
PR 존재와 최신 head를 확인한 뒤에만 생성한다. 사용자 계정의 자기 DM은 Windows 알림을 만들지 않으므로 메시지를 보내지 않고, 인증된 사용자 본인에게 Slackbot 리마인더를 생성한다.

- 현재 Slack 연결의 `slack_create_reminder`를 사용한다. `time`은 현재 시각에서 설정의 `slack_reminder_delay_minutes`만큼 뒤인 Unix timestamp로 계산하며 기본값은 1분이다.
- 같은 `pr_url + head_sha`에 대해 한 번만 생성한다. 성공 결과와 대상 head SHA를 상태 파일에 기록하고 `slack_status: scheduled`로 변경한다. 수정 후 새 head의 검증이 끝나면 새 “재리뷰 요청” 리마인더를 생성한다.
- 생성 실패는 PR을 삭제하거나 재생성할 이유가 아니다. `slack_status: failed`로 남기고 최종 답변에 PR URL을 제공한다. 결과가 불확실하면 자동 재시도하지 않는다.
- 리마인더가 발생하면 사용자는 PR을 검토한 뒤 Slack에서 완료 처리한다. 자동으로 완료하거나 삭제하지 않는다.
- 메시지는 사용자가 무엇부터 확인할지 알 수 있도록 아래의 **리뷰 시작 순서**를 그대로 따른다. 제목은 “리뷰 준비 완료”로 한다. 원본 코드 전체나 환경 변수는 보내지 않는다.
  1. **이전 구조와 현재 구조 차이** — PRD와 실제 diff에서 확인한 핵심 호출 흐름·책임·수명 경계의 Before/After를 1~3개 적는다. 구조 변경이 없으면 억지로 만들지 않고 `구조 변경 없음 — 동작/데이터만 변경`이라고 쓴다.
  2. **PR 확인** — 실제 PR 링크, base/head, 컴파일·테스트·AI 리뷰 결과를 적는다.
  3. **중점적으로 볼 부분** — 회귀 위험, 직렬화·씬·성능·입력·비동기 등 이번 변경에서 사람이 직접 판단해야 하는 항목을 우선순위 순으로 1~3개 적는다. 일반적인 체크리스트를 복사하지 않는다.
- 마지막에는 사용자가 `승인` 또는 `수정 요청`으로 답할 수 있게 다음 행동을 한 줄로 요청한다.

```text
[ECS] 리뷰 준비 완료: <기능명>

1. 이전 구조와 현재 구조 차이
- Before: <기존 호출 흐름·책임·수명>
- After: <현재 호출 흐름·책임·수명>
- 변경 이유: <이 구조를 선택한 근거>
...
3. 중점적으로 볼 부분
- [높음] <가장 중요한 회귀 위험 또는 설계 판단>
- [중간] <직접 확인할 동작 또는 데이터>
- [낮음] <있을 때만 추가>

확인 후 `승인` 또는 수정할 내용을 답해주세요.
```
