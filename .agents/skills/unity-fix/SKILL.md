...
2. **근본 원인** — AGENTS.md 규칙: "오류가 있다"로 끝내지 말고 (1) 왜 발생하는지 코드/씬 구조까지 추적 (2) 실제 문제를 구체적으로 짚고 (3) 해결 방법 제시. 흔한 원인 순서: NullReference(미할당/파괴/실행 순서) → Missing Script(파일명≠클래스명) → 직렬화 손실(FormerlySerializedAs) → 풀 키 불일치(SOPoolData 참조) → 물리(레이어/콜라이더) → 빌드(UnityEditor 누출, 플랫폼 define). 증상만 고치지 말고 **모든 호출자를 grep**해서 공유 지점 하나를 고친다.
