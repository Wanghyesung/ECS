...
플랫폼: **$ARGUMENTS** (없으면 Windows — 주 타겟. 부 타겟 Android)

이 세션이 직접 수행한다.

1. **빌드 전 점검** — `refresh_unity(compile: request)` → `read_console(types: error)` 에러 있으면 중단. `mcpforunity://project/info`로 현재 플랫폼 확인. 빌드 씬 목록(`EditorBuildSettings`) 존재 확인 — 이 프로젝트는 `LobyScene` 하나만 Build Settings에 있고 나머지는 Addressables.
