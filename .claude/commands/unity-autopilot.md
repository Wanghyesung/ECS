...
description: "무인 실행 — 승인된 PRD(.claude/docs/prd/<X>.md, status: approved) 하나를 질문 없이 구현→컴파일→테스트→독립 리뷰→커밋까지 수행하고 PRD에 결과를 기록. autopilot.ps1이 claude -p 로 호출. 대화형에서는 /unity-workflow 를 쓸 것."
user-invocable: true
args: prd_path
---
...
사람이 없다. **절대 질문하지 않는다**(AskUserQuestion 금지). 판단이 필요하면 PRD의 결정/제약을 따르고, 없으면 가장 보수적인 해석을 택해 결과 섹션에 "가정"으로 기록한다.
...
PRD의 Overview / User Flow / Functional Requirements / Technical Architecture / Acceptance Criteria(Given-When-Then)를 읽는다. `.claude/docs/game-design.md` 의 인용 섹션이 있으면 그것도 읽는다. **PRD 범위 밖의 코드는 건드리지 않는다.**

## 2. 탐색 → 구현

1. Grep/Glob으로 비슷한 기존 코드·필드·메서드를 찾아 재사용한다 (새로 만들기 전에 반드시).
2. `.claude/rules/` 전부 준수 (`m_` 헝가리안, UniTask, 풀링, `== null`, SO에 런타임 상태 금지).
...
2. `git add -A -- Assets .claude/docs/prd` (ProjectSettings/Packages 는 절대 add 하지 않는다 — 훅이 차단)
3. `git commit -m "[Auto] <FeatureName> — PRD: .claude/docs/prd/<X>.md"` (커밋 메시지 끝에 `Co-Authored-By: Claude Opus 5 <noreply@anthropic.com>`). 훅이 커밋을 막으면(`DISABLE_HOOK_BLOCK_GIT_COMMIT` 미설정) 커밋 없이 결과만 남기고 그 사실을 Result에 적는다.

**막힘(blocked):**
1. PRD 프런트매터 `status: blocked`, `## Blocked (autopilot <날짜>)` 섹션: 어디서(단계), 왜, 마지막 에러/실패 테스트 원문, 사람이 결정해야 할 것.
2. 코드 변경은 `git stash push -u -m "autopilot blocked: <X>"` 로 치운다 — 다음 PRD가 깨끗한 상태에서 시작해야 하므로. 아침에 `git stash list` 로 확인.
...
- PRD 범위 밖 리팩터링. `known-issues.md` 편집(훅이 승인을 요구해 무인 모드에선 거부됨 — PRD Result 에 적는다).
