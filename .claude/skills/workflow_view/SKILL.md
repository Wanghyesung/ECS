...
- **의사코드·요약 금지.** `.cs` 파일에 그대로 들어갈 코드를 쓴다 — 주석 헤더(`/*/// 제목 / 기능 : ... *///`),
  `m_` 필드 / `_` 매개변수 접두사, 타입 접두사(`f`/`i`/`v`/`q`/`ref`/`SO`/`t`/`e`/`list`/`arr`...), 데이터 컨테이너의 public PascalCase,
  한 줄 if 본문은 다음 줄, `== false`, `++i`, 새 타입 `sealed`, 명시적 접근 제한자까지
