...
- C# 스크립트, 씬, 프리팹, ScriptableObject 또는 프로젝트 설정을 수정하기 전에 영향 파일과 구체적인 변경 코드 또는 diff, 검증 방법을 사용자에게 먼저 제시한다.
- 사용자가 명시적으로 적용을 승인하기 전에는 파일을 수정하지 않는다.
- 읽기 전용 조사와 진단은 승인 없이 수행할 수 있다.
- 승인 후에는 제시하고 승인받은 범위만 적용한다.
