...
- bool 변수·프로퍼티·bool 반환값은 `!value`로 부정하지 말고 `value == false`로 비교하세요. 긍정 여부를 강조해야 하면 `value == true`를 사용하세요. 외부 플러그인 코드를 일괄 변경하지 말고 새 코드와 이번 수정 범위에 적용하세요.
...
- 인스펙터에서 반드시 할당하는 `[SerializeField]`, `[RequireComponent]`가 보장하는 컴포넌트, 초기화 후 종료까지 유지되는 매니저 싱글톤은 호출부마다 null 체크하지 마세요. 누락은 즉시 예외로 드러나야 씬/프리팹 설정을 고칠 수 있습니다.
- 필수 컴포넌트가 없을 때 런타임 `AddComponent`로 조용히 복구하지 마세요. `[RequireComponent]` 또는 씬/프리팹 구성으로 계약을 표현하고 `Awake`나 명시적 `Config`에서 한 번 캐싱하세요.
- null 체크는 선택적 기능, 풀 고갈처럼 정상적으로 실패할 수 있는 반환값, 런타임에 파괴·교체되는 대상, 초기화 전 `OnDisable` 같은 실제 생명주기 경계에만 둡니다.
- `ObjectPoolManager.m_Instance`처럼 부트스트랩 이후 프로세스 종료까지 생존이 보장된 싱글톤은 일반 호출부에서 null 체크하지 않습니다. 싱글톤 자신의 생성·중복 인스턴스 가드만 예외입니다.
