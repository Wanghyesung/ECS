...
/*///////////////////////////////////////////
           LegacyBruteForceSpawnerBrute
목적 : LegacyBruteForceColliderManagerBrute(421d1b0 순수 브루트포스 포팅) 위에서
       실제 부하 조건(총알 다수 × Box 장애물 × 몬스터)을 만드는 스포너.
       GridTestScene0/1의 스포너와 절차/기본값은 동일하고 만드는 컴포넌트 타입만
       Brute 버전이다. BruteForceTestScene 전용.

       각 오브젝트를 SetActive(false)로 만들어두고 SetLayer()까지 끝낸 뒤에야
       SetActive(true)로 활성화한다 - AddComponent 도중 OnEnable(Activate)이
       SetLayer보다 먼저 돌면 전부 레이어 0으로 등록되는 함정이 있어서(GridTestScene0
       실측 중 발견) 처음부터 반영했다.
 *///////////////////////////////////////////
public sealed class LegacyBruteForceSpawnerBrute : MonoBehaviour
{
    private const int LAYER_BULLET = 0;
    private const int LAYER_OBSTACLE = 1;
    private const int LAYER_MONSTER = 2;
    ...
    private void EnsureManager()
    {
        ...
        {
            GameObject go = new GameObject("LegacyBruteForceColliderManagerBrute");
            go.AddComponent<LegacyBruteForceColliderManagerBrute>();
        }
        LegacyBruteForceColliderManagerBrute.Instance.ConfigureLayerMatrix(LAYER_BULLET, LAYER_OBSTACLE, LAYER_MONSTER);
    }
    ...
    private void Spawn()
    {
        ...
        for (int i = 0; i < m_iObstacleCount; ++i)
        {
            ...
            LegacyBruteForceBoxColliderBrute refCollider = go.AddComponent<LegacyBruteForceBoxColliderBrute>();
            refCollider.SetHalfExtent(vHalfExtent);
            refCollider.SetLayer(LAYER_OBSTACLE);
        ...
        for (int i = 0; i < m_iMonsterCount; ++i)
        {
            ...
            LegacyBruteForceCircleColliderBrute refCollider = go.AddComponent<LegacyBruteForceCircleColliderBrute>();
            refCollider.SetRadius(m_fMonsterRadius);
            refCollider.SetLayer(LAYER_MONSTER);
        ...
        for (int i = 0; i < m_iBulletCount; ++i)
        {
            ...
            refCollider.SetLayer(LAYER_BULLET);
